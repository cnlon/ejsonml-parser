...
const ROLLBACK_COMMANDS_KEYS = Object.keys(ROLLBACK_COMMANDS)
...
module.exports = class Parser {
  ...
  static closetag (tagname) {
    ...
    while (stack[0] === COMMAND_TAG) {
      ...
      rollbackCmd = Parser.needRollback(current[1])
      if (!rollbackCmd) {
        continue
      }
      ...
      Parser.rollBack(before[1], rollbackCmd, (roll) => {
        children.pop()
        current[2].push({
          cmd: rollbackCmd,
          value: current[1][rollbackCmd],
        })
        before.push(current[2])
        if (before[2].length === 3) {
          before[2].push({
            cmd: roll,
            value: before[1][roll],
          })
  ...
  static needRollback (attribs) {
    for (let i = 0, l = ROLLBACK_COMMANDS_KEYS.length, cmd; i < l; i++) {
      cmd = ROLLBACK_COMMANDS_KEYS[i]
      if (attribs[cmd] !== undefined) {
        return cmd
      }
    }
  }
